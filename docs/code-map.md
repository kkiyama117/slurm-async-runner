...
| `runner.rs` | `query_job_states_batch`, `query_job_states_batch_with`, `query_job_states_with_exit_code_with` (PR #6), `query_array_task_state_with` (squeue を `<master>_<idx>` キーで叩く per-task probe、PR #12) / `query_array_task_outcome_with` (sacct を **master キー**で叩き、展開された listing から自タスクの `<master>_<idx>` 親行を抽出 — array 全体で 1 キャッシュキーになり sacct spawn を縮約), 内部 `parse_squeue` / `parse_sacct` / `parse_qgroup_l` / `merge_results` / `parse_squeue_array_task` / `parse_sacct_array_task_with_exit_code` |
...
| `qgroup_cache.rs` / `squeue_cache.rs` / `sacct_cache.rs` | `QueryShape` の 3 実装 + type alias。qgroup はキーなし縮退形 (v2.0.0)、squeue summary / sacct exit-code は `-j key1,key2,…` バッチ (squeue は v2.0.0、sacct は PR #19)。キーは `query_cache::JobKey`（plain u64 と array-task `<master>_<idx>` が同一バッチに混在、KUDPC 実機検証 2026-06-12）。squeue argv は PENDING 集約回避のため `-r` 必須。sacct の array finalizer は master キーで到着（array 全体で 1 キャッシュキー — sacct 縮約）。legacy 3 列 sacct はパススルー |
...
| `capture` の失敗時にエラー詳細が空で困る | `src/dispatcher.rs::JobDispatcher::capture` が `CaptureOutput { exit_code, stdout, stderr }` を構造化して返す。`SbatchSpawnError::SubmitFailed::output` には `CaptureOutput::diagnostic()`（stdout + `[stderr]` マーカー + stderr の結合形）が入り、sbatch の `sbatch: error: ...` がそのまま読める |
| 配列ジョブの per-task ステート/exit_code を取りたい | `array_task_id.is_some()` の `SbatchJobHandle` から `refresh()` / `refresh_with_sacct()` を呼ぶ。内部では `query_array_task_state_with` (`squeue -r -j <master>_<idx>`) と `query_array_task_outcome_with` (`sacct -j <master>` — master キーで展開 listing から自タスク行を抽出) が叩かれる (PR #12)。master 集計を返す `qgroup -l` は array task 経路では skip される |
